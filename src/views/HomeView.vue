  ...
  <div class="grid grid-cols-4 gap-4">
    ...
    >
      ...
      <div class="flex-1">
        ...
        <div class="text-2xl font-bold text-gray-900 dark:text-zinc-200">
          {{ users ? users.length : 0 }} <span class="text-teal-500">User</span>
        </div>
        <div class="text-sm mt-1">
          <span class="text-green-600 dark:text-green-400 font-semibold">{{ users ? users.length : 0 }} </span>
    ...
    >
      ...
      <div class="flex-1">
        ...
        <div class="text-2xl font-bold text-gray-900 dark:text-zinc-200">
          {{ asset ? asset.length : 0 }} <span class="text-teal-500">Asset</span>
        </div>
        <div class="text-sm mt-1">
          <span class="text-red-600 dark:text-red-400 font-semibold"
            >{{ asset ? asset.filter((asset) => asset.status !== 'Baik').length : 0 }}
          </span>
    ...
    >
      ...
      <div class="flex-1">
        ...
        <div class="text-2xl font-bold text-gray-900 dark:text-zinc-200">
          {{ maintenances ? maintenances.length : 0 }} <span class="text-teal-500">Maintenance</span>
        </div>
        <div class="text-sm mt-1">
          <span class="text-green-600 dark:text-green-400 font-semibold"
            >{{ maintenances ? maintenances.filter((maintenances) => maintenances.status == 'Selesai').length : 0 }}
          </span>
    ...
    >
      ...
      <div class="flex-1">
        ...
        <div class="text-2xl font-bold text-gray-900 dark:text-zinc-200">
          {{ inspeksis ? inspeksis.length : 0 }} <span class="text-teal-500">Inspeksi</span>
        </div>
        <div class="text-sm mt-1">
          <span class="text-green-600 dark:text-green-400 font-semibold"
            >{{
...
import { UserIcon, CubeTransparentIcon, ClipboardDocumentIcon, Cog6ToothIcon } from '@heroicons/vue/24/outline'
...
const fetchInspeksi = async () => {
  ...
  try {
    ...
    console.log('inspeksi', inspeksis.value)
  } catch (err) {
